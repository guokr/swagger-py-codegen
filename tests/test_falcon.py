...
from swagger_py_codegen.falcon import (
    _swagger_to_falcon_url,
    _path_to_endpoint,
    _path_to_resource_name,
    FalconGenerator
)

def test_swagger_to_falcon_url():
    cases = [
        {
            'url': '/users/{id}',
            'data': {
        ...
        },
        {
            ...
            'expect': (
                '/goods/categories/{category}/price-large-than/{price}/order-by/{order}',
    ...
    for case in cases:
        assert _swagger_to_falcon_url(case['url'], case['data']) == case['expect']
...
def test_process_data():
    ...
    assert schemas.data['validators'][('posts_post_id', 'GET')]['params']['properties']['page']['type'] == 'integer'
